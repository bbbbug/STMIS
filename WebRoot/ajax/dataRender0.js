		...
		success : function(result) {			
			...
			$.each(result,function(index,item){
				...
				html+='<div class="row">'
                    ...
                    +'</div>'
                    +'<div class="col-xs-1">'                    
                	+'<label><input class="select" name="select" id="checkbox'+item.id+'" type="checkbox" value="'+item.id+'"/></label>'                	
                    +'</div>'                    
                    +'</div>';
				html+=modal;
			});
...
function loadEmployeeModal(item){
	...
	modal='<div class="modal fade bs-example-modal-lg" id="viewEmployee'+item.id+'" tabindex="-1"'
		 ...
		 +'</div>'
		 +'<div class="form-group">'
		 +'<label for="input" class="col-sm-2 control-label">员工所属单位</label>'
		 +'<div class="col-sm-8">'
		 +'<input type="text" class="form-control"'
		 +'id="YGSSDW'+item.id+'" value="'+item.bxzd_1+'" placeholder="员工所属单位">'
		 +'</div>'
		 +'</div>'
		 +'<div class="form-group">'
