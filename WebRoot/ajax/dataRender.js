...
$(function(){
	$(".yearEnd").click(function(){
		...
		getAllyearEndSelect(1);
	});
	$(".offlineRecord").click(function(){
		//getAllyearEndSelect1();
		getAllofflineRecordSelect(1);
	});
	
	$(".backRecord").click(function(){
		
		getAllfeedBackFormSelect(1);
	});
...
function showAccountInfo(){
	$.ajax({
		...
		success : function(result){
			console.log(result);
			glyzh=result.extend.managerAccount;	
			//显示管理员账户名称
			$("#accountName").text(glyzh);
		}
	});
 ...
 */
/*获取所有课程目录*/
function getAllCategory(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {			
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#category_pageInfo ul li");//获取分页的分页footer
 ...
 */
/*获取所有课程*/
function getAllCourse(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {	
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#course_pageInfo ul li");//获取分页的分页footer
			page(result,pageInfo_id,pageCurrentNum);
//			var pageNum=pageInfo_id.find("select").val();
			var data;
//			alert("课程分页测试:"+pageCurrentNum);
			data=pageInfo(result,pageInfo_id,pageCurrentNum);
			...
			$.ajax({
				...
				success : function(result) {
					...
					$.each(option, function() {
						...
						if(this.lbbh==data.kclb){
							var html = "<option selected>" + this.lbbh + "</option>";
						}else{
...
//			}else{
//				alert("有课程资源待审核或未通过，请进入资料审核进行查看");
//			}
		}	
	});
}
 ...
 * 教师管理
 */
function getAllTeacher(pageCurrentNum){
	$.ajax({
...
/*获取所有课程资源*/
function getAllResource(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {		
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#courseResource_pageInfo ul li");//获取分页的分页footer
			page(result,pageInfo_id,pageCurrentNum);
//			var pageNum=pageInfo_id.find("select").val();
			var data;
//			alert("课程分页测试:"+pageCurrentNum);
...
function loadEmployeeCreditModal(item){
	...
	modal='<div class="modal fade bs-example-modal-lg" id="viewEmployeeCredit'+item.id+'" tabindex="-1"'
		 ...
		 +'<div class="col-sm-8">'
		 +'<input type="text" class="form-control"'
		 +'id="_YGZH'+item.id+'" placeholder="员工账号" value="'+item.ygzh+'" readonly="readonly">'		
		 +'</div>'
		 +'</div>'
		 +'<div class="form-group">'
 ...
 * 
 * */
function getDeptWithBMBH(ygzh,bmbh){
//	alert(bmbh);
	$.ajax({
		...
		success : function(result) {	
//			alert(JSON.stringify(result));
			var bmmc=result.extend.department.bmmc;
//			alert(bmbc);
			$("#employee #"+ygzh+"_bmbh").html(bmmc);
		}
	})
}
 ...
 * */
/*显示所有员工用户列表*/
function getAllEmployee(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {			
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#employee_pageInfo ul li");//获取分页的分页footer
			page(result,pageInfo_id,pageCurrentNum);
//			var pageNum=pageInfo_id.find("select").val();
			var data;
//			alert("测试:"+pageCurrentNum);
			...
			$.each(data,function(index,item){
				...
				html+='<div class="row">'
                    ...
                    + item.ygdh
                    +'</div>'
                    +'<div class="col-xs-2" id="'+item.ygzh+'_bmbh" style="white-space: nowrap; overflow: hidden;text-overflow:ellipsis;">'
                    + ygssdw
                    +'</div>'
                    +'<div class="col-xs-2">'
                    + ygdlzt
...
//                	+'<button class="btn btn-warning btn-xs" data-toggle="modal" data-target="#viewEmployeeCredit'+item.id+'" onclick="viewEmployeeCredit(\''+item.ygzh+'\',\''+item.id+'\')">成绩</button> '
			...
			$.each(data,function(index,item){
				var ygzh=item.ygzh;
				var bmbh=item.ygdwbh;
//				alert(ygzh);
				getDeptWithBMBH(ygzh,bmbh);
			});
...
/*获取所有管理员信息*/
function getAllManager(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {			
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#manager_pageInfo ul li");//获取分页的分页footer
			page(result,pageInfo_id,pageCurrentNum);
//			var pageNum=pageInfo_id.find("select").val();
			var data;
//			alert("管理员分页测试:"+pageCurrentNum);
			data=pageInfo(result,pageInfo_id,pageCurrentNum);
...
/*获取当前管理员信息 修改时间:2019.9.2 @author chen*/
...
/*获取当前管理员的账户信息 修改时间:2019.9.2 @author chen*/
 ...
 * 员工学分管理
 */
function getAllCourseSelection(pageCurrentNum){
	$.ajax({
		...
		success : function(result) {			
			 ...
			 * 功能:分页
			 */
			var pageInfo_id=$("#courseSelection_pageInfo ul li");//获取分页的分页footer
			page(result,pageInfo_id,pageCurrentNum);
//			var pageNum=pageInfo_id.find("select").val();
			var data;
//			alert("学员成绩测试:"+pageCurrentNum);
			data=pageInfo(result,pageInfo_id,pageCurrentNum);			
